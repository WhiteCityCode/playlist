...
import Html exposing ( Html, Attribute, img , text, p, strong, a, span, main_ )
import Html.Attributes as Hatt exposing ( attribute, style, src, class, href )
import Html.Events exposing ( onClick )
import Json.Decode as D exposing (Decoder, field, string, array, map2)
import Http
import Debug

type alias Model = { videos: Array Video, current: Int, loading: Bool, error: Maybe String}

type Msg = PlayNew Int
    | GotVideos (Result Http.Error (Array Video))

type alias VideoId = String
type alias VideoTitle = String
type alias Video = { id: VideoId, title: VideoTitle }
...
init flags
    = let
        emptyModel: Model
        emptyModel = { videos = Ar.empty, current = 0, loading = True, error = Nothing }
        in
        ( emptyModel
        , Http.get
...
update msg model
    = case msg of
        PlayNew id -> ({model | current = id}, Cmd.none)
        GotVideos (Ok vids) -> ({model | loading = False, videos = vids}, Cmd.none)
        GotVideos (Err e) ->
            let err = case e of
                    Http.BadUrl _ -> "Incorrect URL"
                    Http.Timeout -> "Destination has timed out"
                    Http.NetworkError -> "Could not connect to data source"
                    Http.BadStatus cd-> "Unexpected response status"
                    Http.BadBody _ -> "Could not parse response"
            in
    ...
    , case model.error of 
        Nothing -> if model.loading
            then progress
                ...
                []
            else playlist model.videos model.current
        Just e -> notification
            Danger
            [ style "margin" "20px 100px" ]
            [ text e ]
...
video idx ac v
    = panelBlock ac [ onClick <| PlayNew idx ]
        [ media []
            [ mediaLeft []
            ...
            , mediaContent []
                [ title H5
                    ...
                    , style "color" <| if ac then "#333" else "#777"
...
playlist lst c
    = columns columnsModifiers [style "margin-top" "20px"]
